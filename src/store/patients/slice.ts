...
const initialState: PatientState = {
  ...
  error: {
    ...
    create: { message: null },
    createWithDetails: { message: null },
    update: { message: null },
    delete: { message: null },
  },
};
...
export const patientSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder
      .addCase(createPatientwithdetails.pending, (state) => {
        state.status.createWithDetails = "pending";
        state.error.createWithDetails = { message: null };
      })
      .addCase(createPatientwithdetails.fulfilled, (state, action) => {
        state.status.createWithDetails = "succeeded";
        if (action.payload) {
          state.items.unshift(action.payload.data);
          state.selectedPatient = action.payload.data;
        }
      })
      .addCase(createPatientwithdetails.rejected, (state, action) => {
        state.status.createWithDetails = "failed";
        if (action.payload) {
          state.error.createWithDetails = { message: "Failed to create patient with details" };
        }
      });
