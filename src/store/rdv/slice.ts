...
export const rdvSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      // GET ALL
      .addCase(getAllRDV.pending, (state) => {
        state.status.getAll = "pending";
        state.error.getAll = { message: null };
      })
      .addCase(getAllRDV.fulfilled, (state, action) => {
        state.status.getAll = "succeeded";
        if (action.payload) {
          state.items = action.payload.data;
        }
      })
      .addCase(getAllRDV.rejected, (state, action) => {
        state.status.getAll = "failed";
        if (action.payload) {
          state.error.getAll = { message: "Failed to fetch RDV" };
        }
      });
    ...
    builder
      ...
      .addCase(createRDV.rejected, (state, action) => {
        ...
        if (action.payload) {
          state.error.create = { message: "Failed to create RDV" };
        }
      });

    builder
      // UPDATE
      .addCase(updateRDV.pending, (state) => {
      ...
      .addCase(updateRDV.rejected, (state, action) => {
        state.status.update = "failed";
        if (action.payload) {
          state.error.update = { message: "Failed to update RDV" };
        }
      });

    builder
      // DELETE
      .addCase(deleteRDV.pending, (state) => {
      ...
      .addCase(deleteRDV.rejected, (state, action) => {
        state.status.delete = "failed";
        if (action.payload) {
          state.error.delete = { message: "Failed to delete RDV" };
        }
      });
  },
});
