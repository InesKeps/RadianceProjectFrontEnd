...
import type { DentDto } from "@/types/consultationdatas"
import { toast } from "react-toastify"
import * as yup from "yup"
import useAppDispatch from "@/hooks/useAppDispatch"
import { createDent } from "@/store/dents/actions"
...
export function DataTable<TData, TValue>({
  ...
  const dentInitialValues : DentDto = {
      numero: 0,
      nom: "",
  };
  ...
  const handleSubmitDent = async (
    values: DentDto,
    formikHelpers: FormikHelpers<DentDto>
    ) => {
    formikHelpers.setSubmitting(true);
    const response = await dispatch(createDent(values));
    console.log("error?");    

    if (response.meta.requestStatus === "fulfilled") {
        toast.success("Dent ajoutée avec succès.");
        formikHelpers.resetForm();
    }

    if (response.meta.requestStatus === "rejected") {
        toast.error("Echec d'ajout de la dent.");
    }
    ...
    <section className="relative">
      <Dialog>
        <Card className="bg-[#f7f9fa]">
          <CardHeader>
            <CardTitle>Liste des dents (Enfant-Adulte)</CardTitle>
            <CardDescription>
              <Input
                placeholder="Rechercher une dent...(entrez le nom de la dent)"
          ...
          <CardFooter>
            <div className="flex justify-between items-center w-full gap-4 px-2">
              <div className="flex gap-2 text-sm">
                <p className="font-medium">Nombre total de dents:</p>
                <b>{data.length}</b>
              </div>
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          <DialogTitle className="text-xl text-center">Ajouter une dent</DialogTitle>
          </DialogHeader>
          <Formik initialValues={dentInitialValues} validationSchema={dentSchema} onSubmit={handleSubmitDent}>
          {(formik) => (
          <Form className="flex flex-col gap-6 w-[80%] mx-auto">
            <InputForm label="Numéro de la dent" name="numero" type="number" placeholder="Entrez le numéro de la dent"/>
            <InputForm label="Nom de la dent" name="nom" type="text" placeholder="Entrez le nom de la dent"/>
