...
export function DataTable<TData, TValue>({
  ...
  const handleSubmitSoin = async () => {
    ...
    if (soin && dent && consultationId && !selectedConsultation?.soins?.some(s => s.soin.id === soin.id && s.dent.id === dent.id)) {
      console.log(id);
      ...
      if (response.meta.requestStatus === "rejected") {
          toast.error("Echec d'ajout du soin");
      }
    }else{
      ...
      <Dialog open={openD} onOpenChange={setOpenD}>
        <Card className="bg-[#f7f9fa]">
          <CardHeader className="flex items-center justify-between">
            <CardTitle>Soins prodigués au patient</CardTitle>
            <CardAction>
