...
import { getAllAssurances } from "@/store/assurances/actions";
...
const Facture = () => {
    ...
    const assurances = useAppSelector((state: RootState) => state.assurance.items)
    const ConsultationDetails = useAppSelector((state: RootState) => state.consultation.ConsultationDetails)
    const soinsConsultation = useAppSelector((state: RootState) => state.consultation.ConsultationDetails?.soins);
    ...
    useEffect(() => {
        if (id) {
            ...
            dispatch(getAllAssurances());
        }
    }, [dispatch, id]);
    
    const patientId = ConsultationDetails?.patient.id; 
    const assurance = patientId ? assurances.find(a => a.patiendId === patientId && a.isActive) : undefined; 
    

    const CalculMontantFacture = () => {
        let MontantTotal:number = 0;

        if (soinsConsultation) {
            for (let i = 0; i < soinsConsultation.length; i++) {
                MontantTotal = MontantTotal + soinsConsultation[i].soin.tarif
            }
        }
        ...
        <section ref={printRef} className="flex flex-col gap-16 border border-gray-200 p-4 h-[1170px] w-[827px]">
            ...
            <div className="flex flex-col justify-around p-4 h-[70%]">
                ...
                <div className="flex flex-col items-center gap-4">
                    <p className="text-xl font-bold underline">FACTURE SOINS DENTAIRES</p>
                ...
                <table className="border border-black">
                    ...
                    <tbody>
                        {soinsConsultation?.map((soin, index) => (
                        <tr key={index}>
                            {index === 0 && (
                            <td className="border border-black pl-2" rowSpan={soinsConsultation.length} >
                                {dateHeureFormat}
                            </td>
                            )}
