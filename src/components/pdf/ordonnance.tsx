...
const Ordonnance = () => {
    ...
    useEffect(() => {
        if (id) {
            ...
            dispatch(getConsultationPrescriptions(Number(id)));
        }
    }, [dispatch, id]);
        ...
        <section ref={printRef} className="flex flex-col gap-16 border border-gray-200 p-4 h-[1170px] w-[827px]">
            ...
            <div className="flex flex-col gap-8 p-4 h-[70%]">
                ...
                <div>
                    ...
                    <p className="font-medium">Sexe: {ConsultationDetails?.patient.genre}</p>
                    <p className="font-medium">Téléphone: {ConsultationDetails?.patient.tel}</p>
                </div>
                <h1 className="font-bold text-xl text-center">ORDONNANCE</h1>
                <ul className="text-lg font-normal">
                    {Prescriptions?.map((prescription, index) => (
                    <li className="flex flex-col lg:flex-row gap-1" key={index}>
                        <span>{prescription.nomMedicament} / {prescription.grammage} / {prescription.forme}</span>
                        <span>{prescription.posologie} / {prescription.duree} / {prescription.quantite}</span>
                    </li>
                    ))}
                </ul>
            </div>
